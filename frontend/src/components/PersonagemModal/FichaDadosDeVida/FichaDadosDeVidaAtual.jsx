...
const dadosFormating = (dados) => {
    return dados.map((element, index) => {
        return (<div>{element.qtd}{element.dado} ({element.classe})</div>);
    });
}
...
const FichaDadosDeVida = (props) => {
    ...
    return (
        <Col className="AtributoBase" style={{ fontSize: "28px" }} onClick={handleClick}>
            {props.data[0].qtd}{props.data[0].dado} +
